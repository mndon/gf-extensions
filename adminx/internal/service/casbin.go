...
`
	...
	// 创建 SyncedEnforcer（保持使用传入的适配器 a）
...
func (a *adapterCasbin) SavePolicy(model model.Model) (err error) {
	...
	for ptype, ast := range model["p"] {
		for _, rule := range ast.Policy {
			...
			_, err := dao.AdminCasbinRule.Ctx(a.ctx).Data(line).Insert()
			if err != nil {
				return err
			}
		}
...
func (a *adapterCasbin) LoadPolicy(model model.Model) error {
	var lines []*entity.AdminCasbinRule
	if err := dao.AdminCasbinRule.Ctx(a.ctx).Scan(&lines); err != nil {
		return err
	}
...
func (a *adapterCasbin) AddPolicies(sec string, ptype string, rules [][]string) error {
	lines := make([]*entity.AdminCasbinRule, len(rules))
	for k, rule := range rules {
		lines[k] = savePolicyLine(ptype, rule)
	}
	_, err := dao.AdminCasbinRule.Ctx(a.ctx).Data(lines).Insert()
	return err
}
...
func loadPolicyLine(line *entity.AdminCasbinRule, model model.Model) {
	lineText := line.Ptype
	if line.V0 != "" {
		lineText += ", " + line.V0
	}
...
func savePolicyLine(ptype string, rule []string) *entity.AdminCasbinRule {
	line := &entity.AdminCasbinRule{}
	line.Ptype = ptype
	if len(rule) > 0 {
		line.V0 = rule[0]
...
func rawDelete(a *adapterCasbin, line *entity.AdminCasbinRule) error {
	db := dao.AdminCasbinRule.Ctx(a.ctx).Where("ptype = ?", line.Ptype)
	if line.V0 != "" {
