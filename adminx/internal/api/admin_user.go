...
type UserSearchRes struct {
	g.Meta   `mime:"application/json"`
	UserList []*model.AdminUserRoleDeptRes `json:"userList"`
	ListRes
}
...
type UserGetParamsRes struct {
	g.Meta   `mime:"application/json"`
	RoleList []*entity.AdminRole `json:"roleList"`
}

// SetUserReq 添加修改用户公用请求字段
type SetUserReq struct {
	DeptId   uint64  `p:"deptId"` //所属部门
...
type UserGetByIdsRes struct {
	g.Meta `mime:"application/json"`
	List   []*model.AdminUserSimpleRes `json:"list"`
}
